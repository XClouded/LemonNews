...
class TypeRSS_1_0 extends TypeAbstract {
	...
	public static Feed feed(URL source, Document document) {
		...
		for (int index = 0; index < root.getChildNodes().getLength(); ++index) {
			Element aux = (Element) root.getChildNodes().item(index);
			String nsuri = aux.getNamespaceURI();
			if ((nsuri == null) || nsuri.equals(Constants.RSS_1_0_NS_URI)) {
				String name = aux.getNodeName();
				if (name.equals("item")) {
		...
		if (channel != null) {
			...
			for (int i = 0; i < header.getNodeCount(); i++) {
				...
				if (node instanceof RawElement) {
					...
					if (evalue != null) {
						// Textual element.
						if (ensuri != null && ensuri.equals(Constants.RSS_1_0_NS_URI)) {
						...
						} else if (ensuri != null && ensuri.equals(Constants.DC_NS_URI)) {
							if (evalue != null) {
