...
import com.GreenLemonMobile.feed4j.bean.RawAttribute;
...
class TypeAbstract {
	...
	protected static void populateRawElement(RawElement re, Element e) {
		...
		re.setName(e.getNodeName());
		// Attributes.
		for (int index = 0; index < e.getAttributes().getLength(); ++index) {
			Node attr = e.getAttributes().item(index);
			RawAttribute rawAttribute = new RawAttribute();
			String attrnsuri = attr.getNamespaceURI();
			if (attrnsuri != null && attrnsuri.equals("")) {
				attrnsuri = ensuri;
			}
			rawAttribute.setNamespaceURI(attrnsuri);
			rawAttribute.setName(attr.getNodeName());
		...
		// Contents.
		if (e.getChildNodes().getLength() == 1 && e.getChildNodes().item(0) instanceof Text) {
			String text = ((Text) e.getChildNodes().item(0)).getData();
			if (text.length() > 0) {
				re.setValue(text);
			}
		} else {
	        ...
	        for (int index = 0; index < e.getChildNodes().getLength(); ++index) {
	            Node current = (Node) e.getChildNodes().item(index);
	            if (current instanceof Text) {
	                Text text = (Text) current;
	                String aux = text.getData();
